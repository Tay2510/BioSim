...
for i = 2 : N        
    ...
    p21(i) = p21(i - 1) + delta_t * (0.03 + 0.3 * p53helper(i - 1) + 0.01 * p53killer(i - 1) - 0.2 * p21(i - 1) - 10 * p21(i - 1) * CycE(i - 1) + 1.12 * p21_CycE(i - 1)) + sqrt(delta_t)*randn();
    if p21(i) < 0
        p21(i) = 0;
    end
...
molecules = [p53killer; p53helper; MDM2; ARF; RB; RBp; CycE; p21; p21_CycE];
% select = [1,2,3,4,5,6];
size(timeline)
size(molecules)
