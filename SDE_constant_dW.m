function SDE_constant_dW(T_end, delta_t)
...
for i = 2 : N        
    ...
    DYRK2(i) = DYRK2(i - 1) + delta_t * (-0.0003 * DYRK2(i - 1)) + sqrt(delta_t)*randn();
    if DYRK2(i) < 0
        DYRK2(i) = 0;
    end
    p53helper(i) = p53helper(i - 1) + delta_t * (-DYRK2(i - 1) * ...
        ...
        /(0.1 + p53killer(i - 1))) + sqrt(delta_t)*randn();
    if p53helper(i) < 0
       p53helper(i) = 0;
    end
    ...
    p21(i) = p21(i - 1) + delta_t * (0.03 + 0.3 * p53helper(i - 1) + ...
        ...
        * CycE(i - 1) + 1.12 * ...
        p21_CycE(i - 1)) + sqrt(delta_t)*randn();
    if p21(i) < 0
        p21(i) = 0;
    end
    p21_CycE(i) = p21_CycE(i - 1) + delta_t * (10 * p21(i - 1) * CycE(i - 1) - p21_CycE(i - 1) - 0.32 * p21_CycE(i - 1)) + sqrt(delta_t)*randn();
    if p21_CycE(i) < 0
        p21_CycE(i) = 0;
    end
    RB_E2F1(i) = 1 - E2F1(i);
...
%     
%     hold
%     plot(timeline,molecules(select_ind));
% end
